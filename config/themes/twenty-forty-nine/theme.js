...
// theme-bundles/twenty-forty-nine/script.ts
var STRIP_GEOMETRY = {
  height: 28,
  hoverHeight: 36,
  gap: 3,
...
function enhance(root, data) {
  const page = root.querySelector(".twenty-forty-nine-page") ?? root;
  const undo = [];
  const rows = [];
...
// theme-bundles/twenty-forty-nine/format.ts
var RANGES2 = [
...
// theme-bundles/twenty-forty-nine/template.ts
var HEADLINE = {
...
function services(data, layout) {
  ...
  const body = layout === "cards" ? rows.map((row) => `<section class="service-card">${ornament}${row}</section>`).join("") : `<section class="service-card">${ornament}${rows.join("")}</section>`;
