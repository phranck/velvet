 ...
 * which is every current browser. These files exist for the two cases that
 * still need bitmaps: an older browser falling back to a PNG, and iOS, which
 * only reads `apple-touch-icon`.
 *
 * The mark itself is the letter V from Plaster, the face the wordmark is set
 * in, taken as an outline so the icon needs no font to render.
 *
