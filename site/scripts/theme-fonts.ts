...
import { THEME_FACES, type Face } from "./theme-faces.js";
...
async function copyFaces(bundle: string, faces: Face[]): Promise<number> {
  ...
  await writeFile(
    ...
    `/*\n  The faces this design carries: ${names}.\n\n` +
      `  Written by site/scripts/theme-fonts.ts together with the files it\n` +
...
async function copyIcons(
  ...
  await writeFile(
    ...
    `/*\n  The Phosphor duotone glyphs this design shows, and nothing else.\n\n` +
      `  Written by site/scripts/theme-fonts.ts, which reads the names out of\n` +
...
for (const [theme, faces] of Object.entries(THEME_FACES)) {
  const written = await copyFaces(theme, faces);
  const icons = await copyIcons(theme, defined);
  const names = faces.map((face) => face.family).join(", ");
  console.log(
    `  ok    ${theme}  ${written} face file(s), ${icons} icon(s): ${names}`,
  );
}
