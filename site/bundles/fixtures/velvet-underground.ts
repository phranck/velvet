/**
 * The ordinary installation: five services with three hundred days behind them,
 * one open incident, one window scheduled and one completed.
 *
 * This is the fixture a design is drawn against, and the only one of the eight
 * that looks like a page somebody would publish. The other seven are the cases
 * that break a design rather than the case that flatters it.
 *
 * Shaped against `packages/contracts/src/schemas.ts` rather than invented, so a
 * design exercises the same document a published page does. The conformance
 * suite checks that claim against the real validators; a fixture that drifted
 * from the schema would make a design look correct whilst the product it
 * describes rendered something else.
 *
 * Deterministic, because two people looking at the same design have to see the
 * same page. Random data would also make a screenshot gate impossible, since
 * every run would differ.
 */
