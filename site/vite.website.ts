 ...
 *
 * Unlike the onboarding bundle, this one is never committed. That one lives at
 * the repository root because the setup service serves it, which is what
 * obliges it to match its source on every CI run. The website has no such
 * consumer, so the Pages workflow uploads this directory as an artefact and
