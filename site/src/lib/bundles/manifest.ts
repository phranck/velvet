 ...
 * browser reads TOML. `parseBundleManifest` returns either the manifest or the
 ...
 */
export const BUNDLE_STATES = ["offered", "withdrawn"] as const;
...
function isState(value: string): value is BundleState {
  return (BUNDLE_STATES as readonly string[]).includes(value);
}
 ...
 */
export function parseBundleManifest(raw: unknown, id: string): ManifestResult {
  const errors: string[] = [];
  if (typeof raw !== "object" || raw === null || Array.isArray(raw)) {
  ...
  if (typeof state !== "string" || !isState(state)) {
    errors.push(`state must be ${BUNDLE_STATES.join(" or ")}`);
  }
