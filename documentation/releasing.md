...
- [ ] Cut the release artefact from `apps/setup-service` with `bun run scripts/build-release.ts --type <security|fix|feature> --minimum 1.0.0 --notes scripts/release-notes.md`, and commit the regenerated `src/velvet-release.generated.ts`. Add `--automatic` only for a migration-free security release, which is the one category eligible for unattended installation.
- [ ] `--minimum 1.0.0` is required until #405 is fixed. Without it the artefact records the previous release as the oldest it will install onto, so the release reaches only a page already running the version before it, and every page that missed one is refused. No release has ever changed a schema, so 1.0.0 is what the compatibility block has always described.
