...
**A moving backdrop moves by `transform` and by nothing else.** A compositor animates `transform`, `opacity` and `filter`, and there is no fourth. `background-position` is a paint, so a full-window texture creeping by it repaints the whole window on every frame, for as long as the page is open. Measured on `retro-chassis` at 1728x1080 and a device pixel ratio of 2: three seconds of an untouched page came to 430 paints and 864 rasterisations, against none of either once the same creep was a `transform`, and six expand-and-collapse cycles fell from 940 paints and 4332 rasterisations to 293 and 1923. Two things go with the change, because moving a layer that exactly covers the window uncovers an edge. It needs the travel distance as overhang on the side it moves away from, and it needs its texture drawn at the window's size and put back where the window's top edge is, since an angled gradient takes its direction line from the box it is drawn in and a taller box moves the pattern even where the origin is corrected. `retro-chassis` renders pixel for pixel as it did; `twenty-forty-nine`, whose texture is two angled gradients, differs by 2 of 255 on its faintest lines.
